 ...
 */
public class ProvincesPickerDialog extends AlertDialog implements DialogInterface.OnClickListener,
    ...
    public ProvincesPickerDialog(Context context,
                                 OnDateSetListener callBack,
                                 String title,
                                 String location) {
        this(context, 0, callBack, title, location);
    }
    ...
    public ProvincesPickerDialog(Context context,
        ...
        mProvincesPicker = (NumberPicker) view.findViewById(R.id.provinces);
        mProvincesPicker.setOnValueChangedListener(onProvinceChangeListener);
        mCityPicker = (NumberPicker) view.findViewById(R.id.city);
        mCityPicker.setOnValueChangedListener(onCityChangeListener);

        setNumberPicker(mProvincesPicker);
        setNumberPicker(mCityPicker);

        String[] pStrs = location.trim().split(" ");
        if (pStrs.length > 0) {
            setProvince(pStrs[0]);
        } else {
            setProvince("");
    ...
    public void onClick(DialogInterface dialog, int which) {
        switch (which) {
            case BUTTON_POSITIVE:
                //Log.d("Provinces", "BUTTON_POSITIVE");
                mProvincesPicker.clearFocus();
                mCityPicker.clearFocus();
                if (mCallBack != null) {
                    mCallBack.onDateSet(getProvinceStr());
                }
                break;
            case BUTTON_NEGATIVE:
                //Log.d("Provinces", "BUTTON_NEGATIVE");
                break;
        }
    }
    ...
    private NumberPicker.OnValueChangeListener onCityChangeListener = new NumberPicker.OnValueChangeListener() {
        @Override
        public void onValueChange(NumberPicker picker, int oldVal, int newVal) {
            if (oldVal != newVal) {
                cIndex = newVal;
    ...
    private void setCity(int pIndex, String cityName) {
        ...
        mCityPicker.setDisplayedValues(cityString);
        mCityPicker.setValue(cIndex);
    }
    ...
    private void setProvince(String provinceName) {
        setProvince(getPIndex(provinceName));
    }

    /*private int getCIndex(int pIndex, String cityName){
        int cIndex = 0;
